...
char* type_without_soustype(char* mimetype){
    char* filename = strrchr(mimetype, '/');
    *filename = '\0';
    return mimetype;
}
...
char* get_mimetype(char* extension_file){
    int size_list_mime = 740-1;
    char* mimefile="pas de mimetype";
	if(extension_file != NULL){
		for(int i=0;i<size_list_mime;i++){
			if(strcmp(extension_file,MegaMimeTypes[i][0])==0){
				mimefile= (char*) MegaMimeTypes[i][2];
			} 
		}
	}
